...
def harvestObject(obj,gs,tool=None):
    ...
    while gs.inv != actualInv: #block while inventory mismatch
...
def turn(gs, angle):#executes a turn in terms of an angle
...
def executeCommand(cmd): #executes a command in game
    pyautogui.press('t')
    pyautogui.typewrite(cmd,interval=.05)
    pyautogui.press('enter')
    time.sleep(.2)

def syncLoc(gs): #teleports player to location based on its position in the 2d world model
